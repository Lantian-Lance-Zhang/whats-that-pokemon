...
def upload_file():
    global model_id, custom_model
    if request.method == "POST":
        message = request.form.get("model_id")
        if message:
            custom_model = True
            model_id = message.lower() + ".h5"
        print(message.lower())
...
def wait_upload(worker_id):
    global changed, PATH, result, filename, model_id, custom_model
